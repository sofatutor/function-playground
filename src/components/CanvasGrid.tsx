...
const CanvasGrid: React.FC<CanvasGridProps> = ({ canvasSize, pixelsPerCm, pixelsPerMm, measurementUnit }) => {
  ...
  // Add a buffer to ensure all grid lines are rendered completely
  const numHorizontalCmLines = Math.ceil(canvasSize.height / pixelsPerCm) + 2;
  const numVerticalCmLines = Math.ceil(canvasSize.width / pixelsPerCm) + 2;
  const cmGridLines = [];
  const mmGridLines = [];
  const cmLabels = [];
