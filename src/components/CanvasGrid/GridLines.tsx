...
const GridLines: React.FC<GridLinesProps> = ({ 
  ...
  const minHorizontalLine = Math.floor(-origin.y / pixelsPerUnit) - 10;
  const maxHorizontalLine = Math.ceil((canvasSize.height - origin.y) / pixelsPerUnit) + 10;
  const minVerticalLine = Math.floor(-origin.x / pixelsPerUnit) - 10;
  const maxVerticalLine = Math.ceil((canvasSize.width - origin.x) / pixelsPerUnit) + 10;
